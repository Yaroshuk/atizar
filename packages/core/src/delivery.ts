import type { Destination, WorkflowDescriptor } from './defineWorkflow.js'
...
// resolves to the PRIVATE bound input agent (the caller never names it). Lifted from the
// client (apps/inbox/client/src/deliver.ts) so the server can resolve handoffs too.
