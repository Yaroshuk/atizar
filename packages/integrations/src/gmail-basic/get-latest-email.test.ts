...
describe('getLatestEmail', () => {
  it('returns parsed fields for a message in the inbox', async () => {
    ...
    const res = await getLatestEmail({ getGmail: async () => fakeGmail(msg) })
    expect(res).toEqual({
      threadId: 'thread-1',
  ...
  it('returns { error } when inbox is empty', async () => {
    const res = await getLatestEmail({ getGmail: async () => fakeGmail(null) })
    expect(res).toEqual({ error: 'No emails found in inbox.' })
  })

  it('returns { error } when getGmail throws', async () => {
    const res = await getLatestEmail({
      getGmail: async () => {
        throw new Error('network failure')
      },
    })
    expect((res as { error: string }).error).toMatch(/network failure/)
  })
})
