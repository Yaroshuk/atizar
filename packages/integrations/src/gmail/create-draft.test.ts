...
describe('createDraft', () => {
  it('creates a draft and returns the draftId (deps.gmail injected)', async () => {
    const { gmail, calls } = fakeGmail()
    const res = await createDraft({ threadId: 't1', body: 'Hello there' }, { gmail })
  ...
  it('returns an error when the thread has no From header', async () => {
    ...
    const res = await createDraft({ threadId: 't1', body: 'Hi' }, { gmail: noFrom })
    expect((res as any).error).toMatch(/recipient/i)
  })

  it('returns { error } when no credential is given (makeGmailClient throws)', async () => {
    const res = await createDraft({ threadId: 't1', body: 'Hi' }, {})
    expect((res as any).error).toMatch(/oauth2 credential/)
  })
})
