...
function replyFirst(
  ...
  return [
    ...
    `"${email.subject}".`,
    `Call get_email with { messageId: "${email.messageId}" } to read the full body.`,
    'Then call renderLead with { from, subject, summary } to surface it (summary = one',
    'sentence on what the email asks for). Draft a short, businesslike reply tailored to',
    'the email, then call saveDraft with { threadId, body } — threadId is',
    `"${email.threadId}", body is your drafted reply — to ask the human before saving.`,
    'Do NOT create the draft yet and do NOT send anything. Do not narrate your tool',
    'usage or mention tools/schemas — keep any text brief and user-facing.',
  ].join('\n')
}
