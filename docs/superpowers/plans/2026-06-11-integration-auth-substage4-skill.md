...
- [ ] **Step 1: Read the current SKILL.md fully** + the spec §1/§5 + `HANDOFF.md` sub-stage 1–3 as-built. Confirm the exact symbol names the skill will cite: `AuthSpec` / `ResolvedCredential` / `CredentialResolver` (`@atizar/core`), `resolveCredential` / `registerResolver` / `atizarEnv` (`@atizar/server`), the env names (`ATIZAR_SECRET_KEY`, `ATIZAR_<PROVIDER>_CLIENT_ID/SECRET`, `ATIZAR_<INTEGRATION>_API_KEY`), and that `.env.example` exists. If any name differs from this plan, use the AS-BUILT name (grep to confirm) — do NOT cite a symbol that doesn't exist.

- [ ] **Step 2: Update the "integration contract (FACTS)" block** — replace the credential-related bullets so they state:
  - **Auth is DECLARED, never self-read.** The integration exports an `auth: AuthSpec` (from `@atizar/core`); its functions receive the live credential via `deps.credential` (`ResolvedCredential`). The integration MUST NOT read `process.env` or files for secrets — that is the framework's job (`resolveCredential`).
  - **`AuthSpec.kind` is open:** `none` / `apiKey` / `oauth2` are built-in (framework resolvers); a custom kind ships its OWN `CredentialResolver` registered via `registerResolver(kind, fn)` in userland — **never edit `@atizar/core`** to add an auth method.
