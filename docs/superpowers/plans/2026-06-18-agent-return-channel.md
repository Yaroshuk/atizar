...
1. **`ask`/`answered` transition edges.** (The `work_item_phase` pg-enum value `awaiting_agent` + migration `0004` ALREADY landed in Plan 1 T3 — pulled forward so the core `Phase` add kept the full typecheck green; single-source Phase↔enum. Do NOT re-add it.) `transition.ts` `Edge` union + `EDGES` gain `ask: { from:['active'], to:'awaiting_agent', outcome:'running' }` and `answered: { from:['awaiting_agent'], to:'active', outcome:'running' }`; add `awaiting_agent` to `cancel.from` (a suspended asker must cancel — I10). Test: legal/illegal edge cases via PGlite.

   **CARRY-FORWARD from Plan 1 final review (do these in Plan 2):**
   - **Widen the server resume seam to `ResumePayload`.** `recordReplay.ts` (~:194), `runObserver.ts` (`AgentRuntime`/`RunObserver.resume`, ~:79 & :362), `pipelineService.ts` (~:335/:476) still type their resume seam as `GateResolution`. It compiles today only because `GateResolution` is assignable to `ResumePayload` and only gate payloads flow. The moment an `{kind:'answer'}` payload is routed through `observer.resume`/the recordReplay decorator, these MUST widen to `ResumePayload` — otherwise the answer is silently up-typed to `GateResolution` and each provider's `payload.kind === 'answer'` branch is **unreachable through the wrapper**.
   - **Asks-wiring validation.** An agent that declares an `asks` tool but whose `PromptStrategy` lacks `buildResumeFromAnswer` no-ops silently on answer-resume. Add a validation at `buildAgent`/`createServer` (the only layer that sees `agent.asks` against its `PromptStrategy`) that fails loudly. Neither the provider nor `definePrompt` can detect this (they don't see the agent's `asks`).
